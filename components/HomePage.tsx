...
import { Board, Link as LinkType } from '@/types/board'

export default function HomePage() {
  const [boards, setBoards] = useState<Board[]>([])
  const [newBoard, setNewBoard] = useState<Omit<Board, 'id' | 'links' | 'createdAt'>>({ name: '', description: '', type: 'saved' })
  const [newLink, setNewLink] = useState<Omit<LinkType, 'id' | 'visited' | 'createdAt'>>({ url: '', description: '', deadline: '' })
  const [editingBoard, setEditingBoard] = useState<{ originalBoard: Board, editedBoard: Board } | null>(null)
  const [editingLink, setEditingLink] = useState<{ board: Board, originalLink: LinkType, editedLink: LinkType } | null>(null)
  ...
  const createBoard = () => {
    if (newBoard.name && !boards.some(board => board.name === newBoard.name)) {
    ...
    } else if (boards.some(board => board.name === newBoard.name)) {
      toast({
        title: "Error",
  ...
  const updateBoard = () => {
    if (editingBoard) {
      if (!boards.some(board => board.id !== editingBoard.originalBoard.id && board.name === editingBoard.editedBoard.name)) {
  ...
  const addLink = (boardId: string) => {
    if (newLink.url) {
      const updatedBoards = boards.map(board => {
        if (board.id === boardId) {
          const newLinkWithId: LinkType = { ...newLink, id: uuidv4(), visited: false, createdAt: Date.now() }
          const updatedBoard = {
            ...board,
          ...
          <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => setShowThemeCustomizer(true)}>Customize Theme</DropdownMenuItem>
